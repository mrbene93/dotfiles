...
if [[ "$OS" == "CachyOS Linux" ]]
then
    sudo pacman -Sy --noconfirm amdgpu_top alacritty borg brave-bin coolercontrol coolercontrold corectrl darktable firefox firefox-i18n-de flatpak gamemode gamescope git libreoffice-fresh libreoffice-fresh-de lact localsend lutris moonlight-qt mpv mullvad-vpn nextcloud-client nvtop proton-cachyos protontricks radeontop remmina signal-desktop solaar steam sunshine thunderbird thunderbird-i18n-de ttf-cascadia-code-nerd vlc vorta wine-cachyos winetricks
...
then
    ...
    sudo dnf install --assumeyes 1password alacritty borgbackup brave-browser cascadia-code-nf-fonts coolercontrol coolercontrold coolercontrol-liqctld corectrl darktable firefox firefox-langpacks flatpak gamemode gamescope lact libreoffice libreoffice-help-de libreoffice-langpack-de lutris mpv mullvad-vpn nextcloud-client nvtop protontricks puddletag radeontop remmina solaar steam thunderbird vlc vorta wine winetricks
